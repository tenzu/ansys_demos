...
p = 1.14	#spiral fibre total pitch number (should equal to an integer plus integral multiple of 360/n over 360)
...
def Lsphere(x): #make sure L_sphere is an odd number
    if x % 2 ==0:
        x += 1
    return(x)
L_sphere = Lsphere(L_sphere)
n_sphs = n*L_sphere   #number of spheres in one cylindar
...
local_CSs = np.zeros((f_num,6)) #local cordinate systems (x,y,z,yz,zx,xy)
for i in range(0,f_num,1):
    for j in range(0,6,1):
        if j == 0:
            #r~=r_disk-s*p-min_gap, x between (-r~,r~)
            r_ = r_disk-s*p-min_gap 
            local_CSs[i][j] = random.uniform(-r_,r_)
        elif j == 1:
            local_CSs[i][j] = random.uniform(-math.sqrt(r_**2-local_CSs[i][0]**2),math.sqrt(r_**2-local_CSs[i][1]**2))
        elif j == 2:
            #t~=2*r_disk*td_ratio, z between (-(t~/2-s*p/2),t~/2-s*p/2)
            t_ = 2*r_disk*td_ratio-2*min_gap
            local_CSs[i][j] = random.uniform(-(t_/2-s*p/2),t_/2-s*p/2)
        else:
            local_CSs[i][j] = random.uniform(0,360)
